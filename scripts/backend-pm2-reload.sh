...
pm2 startOrReload ecosystem.config.cjs --only keychain-ifbaps-sync-worker --update-env
...
if ! pm2 describe keychain-ifbaps-sync-worker | grep -q "status.*online"; then
